...
def run_active_learning_adaptive(idx_labels, L, w, y, c):
    """
    Does a one vs all labelling for each label type, and then performs a semi-supervised clustering, followed by an optimal experimental design algorithm (OEDA) which finds the next points to label, these points are then labelled.
    :param idx_labels: on input, index of the data points already labelled, on output index of the data points already labelled and of the newly labelled data points
    ...
    :param c: an object which contains a lot of configuration hyperparameters, generated by the initial argument parser.
    """
    t0 = time.time()
    n, nc = y.shape
    yi = np.zeros((n, 1))
...
def oeda(idx_learned, w, L, y, c, use_stochastic_approximate=True):
    """
    Finds the next points to learn, according to:
