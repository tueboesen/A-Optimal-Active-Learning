...
from active_learning import OEDB, OEDA, update_laplacian
from src.dataloader import Load_MNIST, set_labels
from networks import ResNet, ResidualBlock
from Laplacian import compute_laplacian, create_uobs_from_dataset, SSL_clustering, analyse_probability_matrix, \
    find_weights_from_labels, convert_pseudo_to_prob, analyse_features
from networks_ae import select_network
from optimization import train, train_AE, eval_net
...
def main(c):
    ...
    if c['use_adaptive_active_learning']:
        ...
        optimizerAL = optim.Adam(list(netAL.parameters()), lr=c['lr'])
        nc = MNIST_train.dataset.nc
        #Lets start by having all classes represented by 2 labels
        MNIST_train = set_labels(nc*2, MNIST_train, class_balance=True)
        y = create_uobs_from_dataset(MNIST_train.dataset)
        idx_t = np.nonzero(y[:,0])[0]
        ...
        L = L + 1e-3*identity(L.shape[0])
        ns = 5
        idx_learned = set(idx_t)
        for i in range(c['epochs_AL']):
            if c['use_1_vs_all']:
                # yi = np.zeros((y.shape[0],2))
                # for j in range(nc):
                #     yi[:,0] = y[:,j]
                #     yi[:,1] = np.sum(y[:,np.arange(nc)!=j],axis=1)
                yi = np.zeros((y.shape[0],1))
                for j in range(nc):
                    yi[:,0] = np.sign(y[:,j])
                    w = OEDA(w,L,yi,c['alpha'],c['sigma'],c['lr_OED'],ns,idx_learned)
                    idx = np.nonzero(w)[0]
                    idx_learned.update(idx)
            else:
                w = OEDA(w,L,y,c['alpha'],c['sigma'],c['lr_OED'],ns,idx_learned)
                idx = np.nonzero(w)[0]
                idx_learned.update(idx)
            MNIST_train = set_labels(list(idx_learned), MNIST_train)
            Uobs = create_uobs_from_dataset(MNIST_train.dataset)
            LOG.info('Balanced clustering')
            y = SSL_clustering(c['alpha'], L, Uobs,balance_weights=True)
            analyse_probability_matrix(y, MNIST_train.dataset,LOG,L)
            #train on this data
            py = convert_pseudo_to_prob(y,use_softmax=True)
            weights = find_weights_from_labels(py,list(idx_learned))
